...
/// Arranges an output directory that already holds a theme for a since-renamed scheme, alongside
/// files the builder never generated. Returns the paths the assertions care about.
fn setup_stale_output(
    test_name: &str,
) -> Result<(PathBuf, PathBuf, PathBuf, Vec<PathBuf>, String, String)> {
    let scheme_name = "silk-light";
    let tmp_dir = unique_tmp_dir(test_name)?;
    let template_theme_path = tmp_dir.join("template");
    let template_templates_path = template_theme_path.join("templates");
    let schemes_path = tmp_dir.join("schemes");
    let themes_path = template_theme_path.join("output-themes");
    let nested_path = themes_path.join("nested");

    let (config_file_content, scheme_file_content, template_file_content, rendered_fixture) =
        setup("base16", scheme_name)?;

    fs::create_dir_all(&template_templates_path)?;
    fs::create_dir_all(&schemes_path)?;
    fs::create_dir_all(&nested_path)?;
    write_to_file(
        schemes_path.join(format!("{scheme_name}.yaml")),
        &scheme_file_content,
    )?;
    write_to_file(
        template_templates_path.join("config.yaml"),
        &config_file_content,
    )?;
    write_to_file(
        template_templates_path.join("base16-template.mustache"),
        &template_file_content,
    )?;

    // The scheme this was generated for has since been renamed, so nothing regenerates it.
    let stale_theme_path = themes_path.join("base16-booshnickle-dark.md");
    write_to_file(&stale_theme_path, "Stale theme content")?;

    // None of these were generated by the builder and none may be pruned: the README and the
    // nested theme fail the `base16-` prefix or the `.md` suffix, and dotfiles are never touched.
    let bystander_paths = vec![
        themes_path.join("README.md"),
        themes_path.join("base16-notes.txt"),
        themes_path.join(".base16-hidden.md"),
        nested_path.join("base16-nested.md"),
    ];
    for path in &bystander_paths {
        write_to_file(path, "Not generated by the builder")?;
    }
    ...
    Ok((
        ...
        bystander_paths,
        themes_path
            .join(format!("base16-{scheme_name}.md"))
            .display()
            .to_string(),
        rendered_fixture,
    ))
...
/// Tests that `--prune-stale` deletes themes whose scheme no longer exists
#[test]
fn test_operation_build_prune_stale() -> Result<()> {
    // -------
    // Arrange
    // -------
    let (
        ...
        rendered_theme_path,
        rendered_fixture,
    ) = setup_stale_output("operation_build_prune_stale")?;
    ...
    // Assert
    // ------
    assert_eq!(fs::read_to_string(&rendered_theme_path)?, rendered_fixture);
    assert!(
        stderr.is_empty(),
        "stderr does not contain the expected output"
    );
    assert!(
        !stale_theme_path.exists(),
        "stale theme should have been pruned: {}",
        stale_theme_path.display()
    );
    for path in &bystander_paths {
        assert!(
            path.is_file(),
            "file should not have been pruned: {}",
            path.display()
        );
    }
    assert!(
...
/// Tests that stale themes are left alone unless `--prune-stale` is passed
#[test]
fn test_operation_build_without_prune_stale() -> Result<()> {
    // -------
    // Arrange
    // -------
    let (
        ...
        rendered_theme_path,
        rendered_fixture,
    ) = setup_stale_output("operation_build_without_prune_stale")?;
    ...
    assert!(
        stale_theme_path.is_file(),
        "stale theme should have been left alone: {}",
        stale_theme_path.display()
    );
    for path in &bystander_paths {
        assert!(path.is_file(), "file does not exist: {}", path.display());
    }
    assert!(
        !stdout.contains("Removed stale theme"),
        "stdout does not contain the exptected output"
    );
